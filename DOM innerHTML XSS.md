...
즉, 서버가 XSS 페이로드를 필터링하더라도, 브라우저에서 동적으로 조작된 DOM 요소가 악성 스크립트를 실행하는 것입니다.
서버 사이드 필터링 없이 클라이언트 사이드에서 동적으로 데이터를 처리할 때 발생하며, 클라이언트 측 자바스크립트 코드에서 처리되어 페이지가 동적으로 HTML을 수정하거나 출력할 때 발생합니다. (서버사이드 렌더링 vs 클라이언트 사이드 렌더링)
...
### 예시 : `element.setAttribute('onchange', userInput)`의 실행 결과
