...
**SQL Injection → PHP 코드 실행** 

```
');passthru('cat /etc/passwd');/*`**
```

`passthru()`는 PHP에서 시스템 명령을 실행하는 고위험 함수이며,
`cat /etc/passwd`는 공격자가 가장 먼저 시도하는 명령입니다.

* `'); ... ;/*`
  → 문자열을 닫고, 주석 처리하여 뒤 부분 무시하는 공격자 문법
* `passthru('cat /etc/passwd')`
  → 리눅스 사용자 정보 유출

#### **공격자가 노린 것**

* 코드 인젝션 가능 여부 및 OS 명령 실행 RCE 여부 확인
