...
| **DOM 기반 XSS 우회**         | 서버 사이드 필터링 없이 클라이언트 사이드에서 동적으로 데이터를 처리할 때 발생하는 XSS. URL 파라미터나 쿠키를 통해 전달된 데이터를 클라이언트 측에서 처리. | `<img src="x" onerror="alert(1)">`                                                                           |
...
유니코드 이스케이프 문자를 사용하여 HTML 태그의 의미를 우회할 수 있습니다.
...
XSS 공격은 **웹 애플리케이션의 입력 필터링과 출력 인코딩**에 대한 우회를 통해 발생할 수 있으며, 공격자는 다양한 기법을 사용하여 보안 메커니즘을 피할 수 있습니다. **입력 검증**, **출력 인코딩**, **서버 측 필터링 강화**, **Content Security Policy (CSP)** 등을 적절히 활용하여 XSS 공격을 방어하는 것이 중요합니다.

이러한 우회 기법들은 **보안 점검**을 통해 식별할 수 있으며, 이를 차단하기 위한 **방어책**을 마련하는 것이 필요합니다.
