...
| **DOM 기반 XSS 우회**         | 서버 사이드 필터링 없이 클라이언트 사이드에서 동적으로 데이터를 처리할 때 발생하는 XSS. URL 파라미터나 쿠키를 통해 전달된 데이터를 클라이언트 측에서 처리. | `<img src="x" onerror="alert(1)">` <br> `<img src="x" onerror="alert(1)">`                                                                          |
